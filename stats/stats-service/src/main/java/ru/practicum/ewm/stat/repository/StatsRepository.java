...
public interface StatsRepository extends JpaRepository<Hit,Long> {
    @Query("SELECT new ru.practicum.ewm.stat.dto.StatsDto(h.app, h.uri, COUNT(DISTINCT h.ip)) " +
            "FROM Hit AS h " +
    ...
    @Query("SELECT new ru.practicum.ewm.stat.dto.StatsDto(h.app, h.uri, COUNT(h.id)) " +
            "FROM Hit AS h " +
            "WHERE h.timestamp BETWEEN ?1 AND ?2 " +
            "AND h.uri IN (?3) " +
            "GROUP BY h.app, h.uri " +
